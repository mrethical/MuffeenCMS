...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            <form class="form-horizontal" method="post" action="{{ url('admin/posts') }}">
                {{ csrf_field() }}
                <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10 col-md-4 col-lg-5">
                        <input class="form-control" type="text" id="post-title" name="title" value="{{ old('title') }}" />
                ...
                <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10 col-md-4 col-lg-5">
                        <div class="input-group">
                            <input class="form-control" type="text" id="post-image-name" readonly
                                @if(old('image'))
                                    @foreach($resources as $resource)
                                        @if(intval(old('image')) === $resource->id)
                                        {{  'value="' . $locations['upload'] . '/' . $resource->name . '"' }}
                                        @endif
                                    @endforeach
                                @endif
                                    ">
                            <input type="hidden" id="post-image" name="image" value="{{ old('image') }}">
                ...
                <div class="form-group {{ $errors->has('content') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10 col-lg-11">
                        <textarea readonly class="form-control tinymce" id="post-content" name="content">{{ old('content') }}</textarea>
